...
/*
float calcDiskArea(const in float triangleArea, const in float a, const in float b, const in float c)  {
  //cirscoscritto
...
void main() {
  ...
  */
  ...
  //vec4 p0 = gl_in[0].gl_Position;   vec4 p1 = gl_in[1].gl_Position;   vec4 p2 = gl_in[2].gl_Position;
  
  vec4 p0 = vertexIn[0].posCS;   vec4 p1 = vertexIn[1].posCS;   vec4 p2 = vertexIn[2].posCS;
