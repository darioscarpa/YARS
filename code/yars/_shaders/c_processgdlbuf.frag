...
vec3 computeFragColor( const in vec2 coords, const in bool showSamplingIntensity ) {
	...
	if (albedoEnabled) albedo = receiverData.albedo;
	
	if (doEnabled) occludedDirectLight    = directRadianceSum;
	if (directLightEnabled) directLight   = receiverData.dl;
	...
	vec3 color = ambientLight + directLight + occludedDirectLight + scaledIndirectLight;
...
vec3 aaacomputeFragColor( const in vec2 coords, const in bool showSamplingIntensity ) {
	if (receiverData.depth == 1.0) {
		discard;
	}
	/////////////////////////////////////////////
	...
	for (int i = 0 ; i < numPointLights; ++i) {
		...
		if (triId != -1) {
			pointLight(pointLights[i], surfaceMtl, receiverData.normal, eyeCS, scattered, reflected);
			pyrDirLight[triId] += scattered + reflected; 
		}		
	}	
	...
	for (int i = 0; i < samples; ++i) {	
		...
		vec3 normalizedSample = normalize(kernelBasis * uKernelOffsets[i]);

		float sampleDepth       = emitterPosition.z;
		float samplingPosDepth  = samplePosCS.z;

		float distanceTerm = abs(samplingPosDepth - sampleDepth) < singularity ? 1.0 : 0.0 ;
		//float distanceTerm = min(recToEmiLen,  abs(samplingPosDepth - sampleDepth) ) < singularity ? 1.0 : 0.0;

		float visibility = 1.0 - aoMultiplier * (sampleDepth > samplingPosDepth ? 1.0 : 0.0) * distanceTerm;

		float receiverGeometricTerm = max(0.0, dot(normalizedSample, receiverNormal));

		ambientOcclusion += (1-visibility) * receiverGeometricTerm;
		
		vec3 radiance = visibility * receiverGeometricTerm * pyrDirLight[triId]; //receiverData.dl; //currEmitterData.dl; //vec3(0.5);
		directRadianceSum += radiance;

		vec3 antiradiance = (1.0-visibility) * receiverGeometricTerm * pyrDirLight[triId];
		directAntiRadianceSum += antiradiance;

		//////////////////////////////////
		vec3 delta = receiverPosition - emitterPosition;
		...
		vec3 bounceRadiance = ilMultiplier * currEmitterData.dl * bounceGeometricTerm;						
...
//	for (int i = 0; i < PYRAMID_DIVS; ++i) {
//		directRadianceSum += pyrDirLight[i];
//	}
	...
	directRadianceSum     *= 2.0 * PI / samples;
	directAntiRadianceSum *= 2.0 * PI / samples;
	occluderRadianceSum   *= 2.0 * PI / samples;
...
*/
	/////////////////////////////////////////////	    
    /*
    float scaledAmbientOcclusion = (ambientOcclusion*aoMultiplier);
	float scaledIndirectLight    = indirectLight*ilMultiplier;
	vec3 ambientLight   = albedo * ambientLightLevel * scaledAmbientOcclusion;
    ...
    */		
	...
	if (doEnabled) occludedDirectLight    = radianceSum; //vec3(0);/*scatteredLight;*/
	if (directLightEnabled) directLight   = receiverData.dl;
	if (aoEnabled) scaledAmbientOcclusion = (ambientOcclusion*aoMultiplier);
	if (ilEnabled) scaledIndirectLight    = indirectLight*ilMultiplier;
...
void main() {
	...
	//FragColor = vec4(gdlbufGetDirectLigh(coordsTC), 1.0);		
	//FragColor = vec4(gdlbufGetNormal(coordsTC), 1.0);			
}
