...
layout(location = 0) out vec4 albedoTexData;
layout(location = 1) out vec4 directLightTexData;
...
void main() {
	...
	//MaterialDesc mtl = MaterialDesc(diffuseColor, vec4(0.0), specularColor, vec4(0.0), shininess);	
	MaterialDesc mtl = MaterialDesc(diffuseColor.rgb, vec3(0.0), specularColor.rgb, vec3(0.0), shininess);	
	
	vec3 scattered = vec3(0);
    vec3 reflected = vec3(0);
	calcAllDirectLighting(mtl, n, vertexIn.eyeCS.xyz, scattered, reflected);	
    
	/////////////////////////////////////////////

	directLightTexData.rgb = max(scattered + reflected, 0.0);
	//directLightTexData.rgb = scattered + reflected;
	directLightTexData.a   = vertexIn.diskArea;

	albedoTexData.rgb =  diffuseColor.rgb;
